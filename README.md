...
| `MORPHEUS_API_KEY_<SLOT>` | Discrete per-bot secrets (`COS`, `CTO`, `CISO`, `BUILDER`, `VERIFICATION`, `COMMS`, `CFO`) |
...
Slot ids are lowercase role keys (`cos`, `cto`, `ciso`, `builder`, `verification`, `comms`, `cfo`) matching those roles. The JSON-map escape accepts arbitrary slot keys when needed. Master / usage-only keys are **refused** for chat, embeddings, and list_models.
